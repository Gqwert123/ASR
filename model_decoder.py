...
from model_part import PositionalEncoding
from model_part import PositionwiseFeedForward
from model_part import LayerNorm
from model_part import MultiHeadedAttention
from model_part import DecoderLayer
import numpy
import numpy as np
import six
import time
...
class Decoder(torch.nn.Module):
    ...
    def __init__(self, odim,
        ...
        self.sos = odim - 1 #change label
        self.eos = odim - 1 #change label
        self.ignore_id=-1
    ...
    def forward(self, tgt, enc):
        ...
        x = self.embed(tgt)
        x, tgt_mask, memory, memory_mask = self.decoders(x, None, enc.unsqueeze(0), None)
        if self.normalize_before:
        ...
        if self.output_layer is not None:
            result = torch.log_softmax(self.output_layer(x_), dim=-1).squeeze(0)
        else:
            result = x_.squeeze(0)
        return result
